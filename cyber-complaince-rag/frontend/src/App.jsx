...
export default function App() {
  ...
  // Azure config — NO embedding deployment needed anymore
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: C.muted }}>
            ChromaDB · Local Embeddings · Azure OpenAI GPT · RAG Pipeline
          </p>
        </div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: C.card, borderRadius: 20, padding: "6px 14px", border: `1px solid ${C.border}`, fontSize: 12 }}>
            <StatusDot active={configured} /> Azure GPT
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: C.card, borderRadius: 20, padding: "6px 14px", border: `1px solid ${C.border}`, fontSize: 12 }}>
            <StatusDot active={true} /> Local Embeddings ✓
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: C.card, borderRadius: 20, padding: "6px 14px", border: `1px solid ${C.border}`, fontSize: 12 }}>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "28px 36px" }}>
        ...
        {tab === "config" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
            ...
            <Card>
              <h2 style={{ margin: "0 0 6px", fontSize: 15, fontWeight: 700, display: "flex", alignItems: "center", gap: 10 }}>
                <Badge color={C.purple}>AZURE</Badge> OpenAI Configuration
              </h2>
              <p style={{ margin: "0 0 20px", fontSize: 12, color: C.muted }}>
                Only your GPT deployment is needed — embeddings run locally for free 🎉
              </p>

              {/* Info box */}
              <div style={{ background: "#0d1f1a", border: `1px solid ${C.teal}33`, borderRadius: 10, padding: "12px 14px", marginBottom: 20, fontSize: 12, lineHeight: 1.7 }}>
                <span style={{ color: C.teal, fontWeight: 700 }}>✓ No embedding deployment needed</span><br />
                <span style={{ color: C.muted }}>
                  Text embeddings run locally using <strong style={{ color: C.text }}>sentence-transformers</strong> (free, ~90MB, downloads once).
                  Azure OpenAI is only used for GPT analysis and ranking.
                </span>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {[
                  ...
                  { key: "deployment_name", label: "GPT Deployment Name", placeholder: "gpt-4o  or  gpt-35-turbo" },
              ...
              {[
                {
                  ...
                  onUpload: handleFwUpload,
                  stat: status.indexed
                    ? `${status.record_count.toLocaleString()} records · ${status.framework_count} frameworks indexed`
                    : null,
                },
                {
                  key: "theme", label: "Theme Hierarchy Workbook", icon: "🏷️",
                  desc: "Excel with Main Theme → Sub Theme → Granular Theme columns (optional)",
                  ref: themeRef, file: themeFile, loading: uploading.theme,
                  onUpload: handleThemeUpload,
                  stat: null,
                },
              ].map(({ key, label, icon, desc, ref, file, loading: upl, onUpload, stat }) => (
                <Card key={key}>
                  ...
                  <button onClick={() => ref.current.click()} disabled={!configured || upl}
                    style={{
                      ...
                      cursor: configured && !upl ? "pointer" : "not-allowed",
                      border: `2px dashed ${file ? C.teal : C.border}`,
                      background: "transparent",
                      color: file ? C.teal : C.muted,
                      fontSize: 13, fontWeight: 600,
                      display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                    }}>
                    {upl
                      ? <><Spinner /> Embedding locally & indexing…</>
                      : file
                        ? `✅ ${file.name}`
          ...
          <div style={{ maxWidth: 820, margin: "0 auto" }}>
            <Card>
              ...
              <p style={{ margin: "0 0 24px", color: C.muted, fontSize: 14 }}>
                Describe your client's industry, geography, or risk scenario in plain language.
              </p>
              ...
              <Btn
                onClick={handleQuery}
                disabled={loading || !query.trim() || !status.indexed}
                style={{ marginTop: 20, width: "100%", padding: "15px 24px", fontSize: 15 }}>
                {loading
                  ? <><Spinner /> Embedding locally · Searching ChromaDB · GPT ranking…</>
            ...
            {status.indexed && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginTop: 18 }}>
                {[
                  ...
                  { icon: "⚡", label: "Embeddings",             value: "Local · Free" },
        ...
        {tab === "results" && results && (
          <div>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20 }}>
                ...
                <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", gap: 8, minWidth: 180 }}>
                  ...
                  <div style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginTop: 6 }}>Main Themes</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {results.themes.slice(0, 5).map((t) => <Badge key={t} color={C.teal}>{t}</Badge>)}
                  </div>
                </div>
              </div>
            </div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", flex: 1 }}>
                {allFrameworkTabs.map((fw) => {
                  const cnt = fw === "All"
                    ...
                    : results.results.filter((r) => r.framework === fw).length;
                  return (
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: "#0d0f1e" }}>
                      {[
                        { label: "#",              width: 40 },
                        { label: "Main Theme",     width: 160 },
                        { label: "Sub Theme",      width: 180 },
                        { label: "Granular Theme", width: 180 },
                        { label: "Section ID",     width: 100 },
                        { label: "Requirements",   width: 420 },
                        { label: "Framework",      width: 130 },
                        { label: "Score",          width: 60  },
                      ].map(({ label, width }) => (
                        <th key={label} style={{
                          padding: "12px 14px", textAlign: "left", color: C.muted,
                  ...
                  <tbody>
                    {displayResults.map((row, i) => {
                      // Group visual — shade rows with same main theme the same way
                      const isEven = i % 2 === 0;
                      return (
                        <tr key={i}
                          style={{ borderBottom: `1px solid ${C.border}22`, verticalAlign: "top" }}
                          onMouseEnter={(e) => e.currentTarget.style.background = "#15182a"}
                          onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>

                          {/* # */}
                          <td style={{ padding: "12px 14px", color: C.muted, textAlign: "center", fontSize: 11 }}>
                            {row.rank}
                          </td>
                          ...
                          <td style={{ padding: "12px 14px" }}>
                            {row.sub_theme ? (
                              <span style={{
                                display: "inline-block",
                                background: C.pink + "18", color: C.pink,
                                padding: "4px 10px", borderRadius: 6,
                          ...
                          <td style={{
                            ...
                            lineHeight: 1.6, color: C.text, fontSize: 12,
                          }}>
                            {row.requirements}
                          </td>
                          ...
                          <td style={{ padding: "12px 14px" }}>
                            <span style={{
                              background: "#1e2235", color: C.text,
                              padding: "4px 10px", borderRadius: 6,
                          ...
                          <td style={{ padding: "12px 14px", textAlign: "center" }}>
                            <span style={{
                              ...
                              color: row.relevance_score >= 8 ? C.green
                                   : row.relevance_score >= 5 ? C.amber
                                   : "#ef4444",
                            }}>
                              {row.relevance_score}
                            </span>
            ...
            <p style={{ marginTop: 10, fontSize: 12, color: C.muted, textAlign: "right" }}>
              Showing {displayResults.length} of {results.total} matched requirements
            </p>
          </div>
        )}
